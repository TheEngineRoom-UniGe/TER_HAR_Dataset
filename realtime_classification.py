...
sloth = TER_sloth(model, window_size=window_size, class_size=output_dim, feature_size=synchronized_sequences.shape[1], rho=Rho, tau=Tau, c=C)
...
while window_idx < synchronized_sequences.shape[0]:
    ...
    pred, time = sloth.classify_offline(window_tensor)
    current_action = np.argmax(pred)
    current_prob = pred[current_action]
    # print(action_names[current_action], current_prob)
    if not in_action:
